...
public class Spaceship extends BaseActor {
    ...
    public Spaceship(float x, float y, Stage stage) {
        ...
        forcefield = new ForceField(0, 0, stage);
        ...
        addActor(forcefield);
        forcefield.setPosition(-getWidth()/2, -getHeight()/2);
        forcefield.setVisible(false);
    }
    ...
    public void setInvincible(boolean invincible) {
        if (invincible) {
            if (!forcefield.isVisible()) {
                forcefield.setVisible(true);
            }
        } else {
            forcefield.setVisible(false);
        }
